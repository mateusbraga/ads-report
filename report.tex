...
Our project is concerned with the subject of consistency in distributed systems and we study the design of Etcd and evaluate its consistency guarantees in the middle of network partitions. It fits in the scheme of verification and validation and its goal of ensuring a system provides the properties it is expected to.
...
Jepsen’s previous work has shown that etcd needed special care to deal with network partitions. In its new version, Etcd has taken measures to make it clear what it is necessary to have linearizability. We would like to rerun the Jepsen tests and also include new network conditions such as: asymmetric partitions, intermittent connectivity, isolated master, and unsynchronized clocks.
...
Consistency can be a very confusing concept. Not only it has different meanings in different areas (database, system, and computer architecture communities), it interacts with other properties that themselves have different names (i.e. before-or-after atomicity and isolation). Nonetheless, consistency is an important concept that drives the differences of many different storage system designs. 

A consistency guarantee dictates what values a read can return based on the order and the instant in time that the operations start and end. A property like consistency is part of the function of the storage system and an application that uses the system may depend on its guarantees to work correctly.

An useful way to represent the function of a system is through a model. A model of a system can be much simpler than its implementation, and still capture the essential properties of the system. This is because from a perspective of outside the system, we only need to consider events that are exposed through the system interface. A fault-tolerant distributed system model, for example, can be oblivious about the data replicas and of failures events tolerated by the implementation, since they are, by definition, not observed by the user~\cite{avizienis2004basic}. 
...
There are two methods to verify and validate a system’s consistency guarantees. The first involves formally proving that the implementation design always satisfy the consistency model, in other words, it proves that it will never executes a sequence of events not allowed by the consistency model. Formal methods like this are usually not done in the industry and believed to require large amounts of time and hard to find expertise. Nevertheless, Amazon has reported successful stories of using TLA+, a formal specification language, in the design and verification of its distributed storage systems~\cite{newcombe2014use}.

The second method is a test approach that tries to find an execution that is not allowed by the consistency model. Even though this method does not prove correctness if it does not find such incorrect execution, it is still very expensive and involves searching for a sequence of the events that is allowed by the model and equivalent to the events produced by each execution. This search space is huge and contains every possible order of the events observed concurrently by different users of the system. This is the approach taken by Knossos/Jepsen. 
...
Model Checking
By answering whether an execution trace is acceptable in a consistency model, Knossos is effectively performing the job of a model checker for the context of consistency models. Since a model checker is also necessary in the formal methods approach, it is important to clarify the differences. 

In the Knossos approach each execution trace is proved to satisfy linearizability independently. Even though it can say for sure whether one particular execution trace conforms to the model, it can’t say much about the other possible traces the system can execute. In a formal method approach, the design of the implementation is proved to always execute valid execution traces. Since the design still needs to be implemented, the formal method approach can’t say for sure that an implementation implements the verified model. \cite{newcombe2014use} argues that a formal proof is still worthwhile due to the increase in understanding of the design resulting from the formal proof specification process, and that this increases the chances of getting the code right.
